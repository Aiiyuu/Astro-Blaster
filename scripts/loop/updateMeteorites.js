/**
 * updateMeteorites.ts
 *
 * Updates the position of each meteorite and removes any that go off-screen.
 */
import config from '../config.js';
export default function updateMeteorites(meteorites) {
    // Iterate backwards to safely remove meteorites while looping
    for (let i = meteorites.length - 1; i >= 0; i--) {
        const meteorite = meteorites[i];
        // If the meteorite is ready to be removed, remove it from the meteorites array.
        if (meteorite.getReadyToBeRemoved()) {
            meteorites.splice(i, 1);
            continue; // Skip the rest of the loop for this meteorite.
        }
        // Update meteorite position/state
        meteorite.update();
        const pos = meteorite.getPosition();
        // Remove meteorite if it goes outside the canvas boundaries
        if (pos.x < -1000 || pos.x > config.game.canvasWidth + 500 ||
            pos.y < -1000 || pos.y > config.game.canvasHeight + 500) {
            meteorites.splice(i, 1);
        }
    }
}

//# sourceMappingURL=updateMeteorites.js.map
