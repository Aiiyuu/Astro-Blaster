...
export default function detectPlayerMeteoriteCollision({ game, player, meteorites }) {
    ...
    meteorites.forEach((meteorite, mIndex) => {
        ...
        const dx = playerPos.x - meteoritePos.x;
        const dy = playerPos.y - meteoritePos.y;
        const distance = Math.sqrt(dx * dx + dy * dy);
        // Assuming the radius of the player is proportional to their scale (or use fixed values)
        const playerRadius = player.getRadius(); // Get the actual radius of the player
...
//# sourceMappingURL=detectPlayerMeteoriteCollision.js.map
